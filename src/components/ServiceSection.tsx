...
export const ServiceSection: React.FC<ServiceSectionProps> = ({ keywordConfig }) => {
  ...
  const dynamicSubTexts: Record<string, string> = {
    "외벽방수": "김해 외벽방수는 외벽 크랙, 조인트, 창호 주변 틈으로 유입되는 빗물 경로를 확인한 뒤 보수 범위와 방수 공정을 정하는 작업입니다.",
    "외벽발수": "김해 외벽발수는 외벽 표면의 수분 흡수와 오염 상태를 확인한 뒤 발수제 적용이 필요한지 판단하는 작업입니다.",
    "옥상방수": "김해 옥상방수는 노후 방수층, 바닥 균열, 배수구 주변 손상을 확인하고 부분 보수와 전체 방수 여부를 판단하는 작업입니다.",
    "지붕방수": "김해 지붕방수는 판넬, 기와, 슬라브 등 지붕 구조와 접합부 상태에 따라 누수 취약 부위를 보강하는 작업입니다.",
    "외벽도색": "김해 외벽도색은 도막 박리, 오염, 바탕면 손상 상태를 확인한 뒤 외벽 보호와 미관 개선을 위한 도장 공정을 정하는 작업입니다.",
    "옥상누수": "김해 옥상누수는 방수층, 배수구, 파라펫 접합부 등 실제 유입 가능 지점을 함께 확인해야 보수 범위를 정할 수 있습니다.",
    "외벽누수": "김해 외벽누수는 실내 물자국과 외벽 크랙, 창호 주변, 조인트 부위를 함께 확인해 빗물 유입 경로를 판단하는 작업입니다.",
